...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeInternalPage in_page = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, perm);
		Iterator<BTreeEntry> iterator = new BTreeInternalPageIterator(in_page);
		BTreeEntry entry;
		...
		// what is field?
		// find the left - deep - most tree
		if(f == null){
			return this.findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, null);
		}

		while(iterator.hasNext()){
			BTreeEntry cur_node = iterator.next();
			// 如果遍历到key >= field 的点 就停止
			if(cur_node.getKey().compare(Op.GREATER_THAN_OR_EQ, f)){
				break;
			}
			else{
				entry = cur_node;
		...
		BTreePageId nextPid = f.compare(Op.LESS_THAN, entry.getKey()) ? entry.getLeftChild() : entry.getRightChild();
		//System.out.println(this.getPage(tid, dirtypages, nextPid, perm).getClass());
		return  this.findLeafPage(tid, dirtypages, nextPid, perm, f);
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage siblingLeafPage = (BTreeLeafPage) this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF);

		BTreeLeafPageIterator iterator = new BTreeLeafPageIterator(page);
		int numPages = page.getNumTuples();
		// find the 1/2  tuples
		for(int i = 0; i < numPages/2 && iterator.hasNext(); i++){
			iterator.next();
		}
		// add the half to the empty sibling Leaf Page
		while(iterator.hasNext()){
			Tuple tuple = iterator.next();
			page.deleteTuple(tuple);
			siblingLeafPage.insertTuple(tuple);
		}

		// GET a parent field key and put it in upper level
		Field midField = siblingLeafPage.iterator().next().getField(this.keyField);
		BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midField);
		// 不要忘了把原本的page 也set parent
		siblingLeafPage.setParentId(parentPage.getId());
		page.setParentId(parentPage.getId());
		// 将key 复制放入parent 注意与 internal的区别
		BTreeEntry siblingParentEntry = new BTreeEntry(midField, page.getId(), siblingLeafPage.getId());
		parentPage.insertEntry(siblingParentEntry);
		// previous leaf <-> #1
		// now leaf <-> sib <-> #1
		// 双向链表 !
		siblingLeafPage.setRightSiblingId(page.getRightSiblingId());
		page.setRightSiblingId(siblingLeafPage.getId());
		siblingLeafPage.setLeftSiblingId(page.getId());
		siblingLeafPage.setParentId(parentPage.getId());

		if (siblingLeafPage.getRightSiblingId() != null) {
			BTreeLeafPage rightSiblingLeafPage = (BTreeLeafPage) this.getPage(tid, dirtypages, siblingLeafPage.getRightSiblingId(), Permissions.READ_WRITE);
			rightSiblingLeafPage.setLeftSiblingId(siblingLeafPage.getId());
			dirtypages.put(rightSiblingLeafPage.getId(), rightSiblingLeafPage);
		}
		dirtypages.put(page.getId(), page);
		dirtypages.put(siblingLeafPage.getId(), siblingLeafPage);
		dirtypages.put(parentPage.getId(), parentPage);

		return field.compare(Op.LESS_THAN_OR_EQ, midField) ? page : siblingLeafPage;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage siblingInternalPage = (BTreeInternalPage) this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);

		// find middle entry
		int mid = page.getNumEntries() >> 1;
		BTreeInternalPageIterator iter = (BTreeInternalPageIterator) page.iterator();

		for (int i=0; i<mid && iter.hasNext(); i++) {
			iter.next();
		}
		BTreeEntry midEntry = iter.next();

		// update left/right internal page
		page.deleteKeyAndRightChild(midEntry);
		while (iter.hasNext()) {
			BTreeEntry entry = iter.next();
			page.deleteKeyAndRightChild(entry);  // delete must be first because insert op will update record id
			siblingInternalPage.insertEntry(entry);
		}

		this.updateParentPointers(tid, dirtypages, siblingInternalPage);

		// update relationship between related pages
		BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midEntry.getKey());
		midEntry.setLeftChild(page.getId());
		midEntry.setRightChild(siblingInternalPage.getId());
		parentPage.insertEntry(midEntry);
		siblingInternalPage.setParentId(parentPage.getId());
		page.setParentId(parentPage.getId());

		// update dirty pages
		dirtypages.put(page.getId(), page);
		dirtypages.put(siblingInternalPage.getId(), siblingInternalPage);
		dirtypages.put(parentPage.getId(), parentPage);

		return field.compare(Op.LESS_THAN, midEntry.getKey()) ? page : siblingInternalPage;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		BTreeLeafPage rightPage = (isRightSibling == true) ? sibling : page;
		BTreeLeafPage leftPage = (isRightSibling == true) ? page : sibling;
		int numOfTuples = rightPage.getNumTuples() + leftPage.getNumTuples();
		BTreeLeafPageIterator leftIter = (BTreeLeafPageIterator) leftPage.iterator();
		int leftidx = 0;
		// find the index which need to be stolen
		while(leftIter.hasNext() && leftidx < numOfTuples / 2){
			leftidx++;
			leftIter.next();
		}
		...
		// steal elements
		if (leftidx < numOfTuples / 2) {  // steal some elements from right page to left page
			BTreeLeafPageIterator rightIter = (BTreeLeafPageIterator) rightPage.iterator();
			for (int i=leftidx; i< numOfTuples / 2 && rightIter.hasNext(); i++) {
				Tuple tuple = rightIter.next();
				rightPage.deleteTuple(tuple);
				leftPage.insertTuple(tuple);
			}
		} else if (leftIter.hasNext()) {  // steal some elements from left page to right page
			while (leftIter.hasNext()) {
				Tuple tuple = leftIter.next();
		...
		// update relationship
		entry.setKey(rightPage.iterator().next().getField(this.keyField()));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		// insert parent entry to right page
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(entry);

		// move left pages to right
		int steal = (leftSibling.getNumEntries() - page.getNumEntries() - 1) / 2;
		BTreeInternalPageReverseIterator leftIter = (BTreeInternalPageReverseIterator) leftSibling.reverseIterator();
		for (int i=0; i<steal && leftIter.hasNext(); i++) {
			entry = leftIter.next();
			leftSibling.deleteKeyAndRightChild(entry);
			page.insertEntry(entry);
		}

		// update parentEntry value
		entry = leftIter.next();
		leftSibling.deleteKeyAndRightChild(entry);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// insert parentEntry to left page
		BTreeEntry entry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
		page.insertEntry(entry);

		// move right pages to left
		int steal = (rightSibling.getNumEntries() - page.getNumEntries() - 1) / 2;
		BTreeInternalPageIterator rightIter = (BTreeInternalPageIterator) rightSibling.iterator();
		for (int i=0; i<steal && rightIter.hasNext(); i++) {
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeLeafPageIterator iterator = (BTreeLeafPageIterator) rightPage.iterator();
		while (iterator.hasNext()) {
			Tuple toDelete = iterator.next();
			rightPage.deleteTuple(toDelete);
			leftPage.insertTuple(toDelete);
		}

		BTreePageId newRightSibId = rightPage.getRightSiblingId();
		leftPage.setRightSiblingId(newRightSibId);

		if (newRightSibId != null) {
			BTreeLeafPage rightSiblingPage = (BTreeLeafPage) this.getPage(tid, dirtypages, newRightSibId, Permissions.READ_WRITE);
			rightSiblingPage.setLeftSiblingId(leftPage.getId());
			dirtypages.put(newRightSibId, rightSiblingPage);
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// move tuples in right page to left page
		...
		BTreeInternalPageIterator iter = (BTreeInternalPageIterator) rightPage.iterator();
		while (iter.hasNext()) {
			BTreeEntry entry = iter.next();
		...
		// delete right page and related parent entry
		this.setEmptyPage(tid, dirtypages, rightPage.getId().pageNumber());
		this.deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
